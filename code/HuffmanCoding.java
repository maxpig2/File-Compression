...
public class HuffmanCoding {
    ...
    public static Map<Character, String> constructTree(String text) {
        ...
        for (Map.Entry<Character, Integer> entry : frequencyMap.entrySet()) {
            queue.add(new HuffmanTree.Node(null,null,entry.getKey(), entry.getValue()));
        }

        while (queue.size() > 1) {
            HuffmanTree.Node node1 = queue.poll();
            HuffmanTree.Node node2 = queue.poll();
            char highestChar;
            if (Character.compare(node1.character,node2.character) > 0){
                highestChar = node2.character;
            } else {
                highestChar = node1.character;
            }


            queue.add(new HuffmanTree.Node(node1,node2,highestChar, node1.frequency + node2.frequency));
        }
        tree = new HuffmanTree();
        tree.root = queue.peek();
        tree.checkNode("",tree.root,codeMap);
        return codeMap;
    }
    ...
    public static String encode(String text) {
        ...
        Map<Character, String> codeMap = constructTree(text);

       for (Map.Entry<Character,String> entry : codeMap.entrySet()){
         //  System.out.println("Character:" + entry.getKey() + " Value:" + entry.getValue() + " Integer:" + (int)entry.getKey());
       }
        ...
        for (int i = 0; i < lengthText; i++) {
          ...
          //  System.out.println("Character:" + c + " Integer:" + ((int)c));
